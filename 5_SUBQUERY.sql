...
-- 2) 급여가 3047662.60869565217391304347826086956522 보다 많이 받는 직원의 사번,이름,직급코드,급여 조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 3047662.60869565217391304347826086956522;

-- 1) + 2) 평균 급여보다 많은 급여를 받고 있는 직원의 사번, 이름, 직급코드, 급여 조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
-- 1) + 2) 노옹철 사원의 급여보다 많이 받는 직원의 사번, 이름, 부서코드, 직급코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, SALARY
FROM EMPLOYEE
...
-- 서브쿼리는 SELECT, WHERE, HAVING, FROM 절에서도 사용 가능
...
-- IN / NOT IN : 여러 개의 결과 값 중에서 한 개라도 일치하는 것이 있으면 / 없다면
...
WHERE DEPT_CODE = (SELECT DEPT_CODE
      ...
      AND EMP_NAME != (SELECT EMP_NAME
                       FROM EMPLOYEE
                       WHERE SUBSTR(EMP_NO, 8, 1) = 2 AND ENT_YN = 'Y'); -- 퇴사한 사람 제외
