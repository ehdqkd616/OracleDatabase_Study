...
-- 목적 : 데이터 무결성 보장
--       데이터 무결성 : 데이터의 정확성, 일과넝, 유효성이 유지되는 것
...
INSERT INTO USER_NOCONST
VALUES(2, NULL, NULL, NULL, NULL, '010-1111-2222', 'NULL@KH.OR.KR');
...
-- UNIQUE 제약 조건 : 컬럼 입력 값에 대해서 중복을 제한하는 제약 조건
-- 컬럼레벨, 테이블레벨
...
CREATE TABLE USER_UNIQUE3(
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    UNIQUE(USER_NO, USER_ID) -- 제약조건에 컬럼두개 들어가면 두 컬럼 다
                             -- 겹칠때만 제한함 컬럼 하나는 겹쳐도 상관없음
);
...
--  테이블에 들어갈 컬럼명           NOT NULL_TABLE명_컬럼명
    TEST_DATA2 VARCHAR2(20) CONSTRAINT UK_TEST_DATA2 NOT NULL,
--  컬럼명               UNIQUE(UK아니면UQ)_TABLE명_컬럼명
...
CREATE TABLE USER_PRIMARYKEY2(
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    CONSTRAINT PK_USERNO_USERID PRIMARY KEY(USER_NO, USER_ID) -- 제약조건에 이름붙이기
);
...
INSERT INTO USER_PRIMARYKEY2
VALUES(2, 'user01', 'pass03', '유관순', '여', '010-3333-4444', 'YOU123@kh.or.kr');
...
-- ON DELETE RESTRICTED(삭제 제한)이 기본 지정되어 있음
...
CREATE TABLE USER_FOREIGNKEY3(
    ...
    -- ON DELETE CASCADE : 부모 키 삭제 시 자식 키도 같이 삭제(행까지)
);
...
-- CHECK 제약조건 : 컬럼에 기록되는 값에 조건을 설정할 수 있음(변하는 값, 함수 사용 불가능)
...
-- 5명 이상 INSERT할 것
ALTER TABLE USER_TEST ADD CONSTRAINT PK_USER_NO2 PRIMARY KEY(USER_NO);
ALTER TABLE USER_TEST ADD CONSTRAINT UK_USER_ID UNIQUE(USER_ID);
ALTER TABLE USER_TEST ADD CONSTRAINT NN_USER_PWD CHECK(USER_PWD IS NOT NULL);
-- ALTER로 NOT NULL 이름 붙이려면 CHECK에 IS NOT NULL로 해야함
ALTER TABLE USER_TEST ADD CONSTRAINT UK_PNO UNIQUE(PNO); 
ALTER TABLE USER_TEST ADD CONSTRAINT NN_PNO CHECK(PNO IS NOT NULL);
ALTER TABLE USER_TEST ADD CONSTRAINT CK_GENDER CHECK(GENDER IN ('남','여'));
ALTER TABLE USER_TEST ADD CONSTRAINT NN_STATUS CHECK(STATUS IS NOT NULL);
ALTER TABLE USER_TEST ADD CONSTRAINT CK_STATUS CHECK(STATUS IN ('Y','N'));
...
VALUES (1,'ABC_ID1','ABC_PW1','111111-2222222','남','010-1234-5678','경기도 어딘가','N');

INSERT INTO USER_TEST
VALUES (2,'ABC_ID2','ABC_PW2','222222-1111111','남','010-1234-5678','경기도 어딘가','N');
