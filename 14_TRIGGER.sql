            ...
            STATEMENT TRIGGER
                SQL문에 대해 한 번만 실행(DEFAULT)
                    EX. UPDATE EMPLOYEE SET SALARY = 0;
                        여러 행에 대해 자료가 변경되지만 한 번만 트리거 실행
    표현식
...
-- EMPLOYEE테이블에 사원이 추가되면 '신입사원이 입사했습니다'를 문구 출력되는 TRG_01 트리거 생성
...
VALUES(905, '길성춘', '690512-1151432', 'GIL_SC@KH.OR.KR', '01077778777', 'D5', 'J3', 'S5', 3000000, 0.1, 200,
...
FOR EACH ROW
BEGIN 
    IF :NEW.STATUS = '입고' -- STATUS컬럼에 새로 들어온 값이 '입고'면
    THEN 
        UPDATE PRODUCT
